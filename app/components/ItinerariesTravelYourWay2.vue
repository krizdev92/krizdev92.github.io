  ...
  <section ref="sectionRef" class="relative w-full pt-10 md:pt-14 pb-4 md:pb-8 flex flex-col overflow-hidden" :style="{ backgroundColor: itineraryCatData.backgrounds.sectionTint }">
    ...
    <div class="relative z-10 w-full max-w-[90rem] mx-auto px-6 md:px-12 lg:px-18 flex justify-center mb-6 lg:mb-8">
      <div class="max-w-5xl flex flex-col items-center text-center">
        ...
        <h4 class="itin-tra-header-elem text-3xl lg:text-4xl tracking-normal leading-[1.2] text-[#703e19] capitalize mb-3 md:mb-4" style="font-family: 'Playfair Display', 'Cinzel', 'Optima', serif; font-weight: 300;">
          <span class="font-normal" v-html="itineraryCatData.header.titleMain"></span>
          <span class="font-normal text-[#c47551] italic"><br />{{ itineraryCatData.header.titleItalic }}</span>
        </h4>
        <p class="itin-tra-header-elem font-sans text-sm md:text-[15px] text-[#1A1A1A] font-light leading-relaxed max-w-2xl">
...
<!-- ================= CATEGORY LISTINGS ================= -->
    <div class="relative z-10 w-full max-w-[90rem] mx-auto px-0 md:px-8 lg:px-12 xl:px-16 flex flex-col gap-4 md:gap-6">
      ...
      <!-- TRAVEL YOUR WAY block (ADDED 'relative') -->
      <div :id="itineraryCatData.travelYourWay.id" class="itin-tra-group-elem w-full flex flex-col mb-2 md:mb-4 relative">
        ...
        >
          ...
          <NuxtLink 
            ...
            class="custom-tyw-card group flex flex-col relative shrink-0 scroll-snap-align-start rounded-[1.25rem] overflow-hidden w-[85vw] sm:w-auto h-full border border-transparent"
            :style="{
        ...
        <div 
          v-if="itineraryCatData.travelYourWay.items.length > 1"
    ...
    <div class="itin-tra-cta-elem w-full mt-2 md:mt-6 pt-2 flex flex-col items-center gap-8 md:gap-10 relative z-10">
      <NuxtLink :to="itineraryCatData.bottomCta.url" class="group relative inline-flex items-center gap-4 px-6 md:px-8 py-3 md:py-4 mb-0 border border-[#1A1A1A]/20 hover:border-[#703e19] transition-colors duration-500 overflow-hidden rounded-sm">
